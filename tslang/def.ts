import { STRTYPE_DEF_STACK, FUNCTION_DEF_STACK, INTTYPE_DEF_STACK, __DEBUG__, errorf } from "./lexer.ts";
...
function decode_value(content:any, expected:types, exep?:boolean): any {
    ...
    if (located === false) errorf(content, `Undefined reference to ${expected}`, expected);
    return returnvalue;
}
...
export { decode_value, isfunction };