...
async function parsearraystr(array:string, name:string, type:any, push?:order):Promise<any> {
...
async function parsearrayint(array:string, name:string, type:any, push?:order) {
    const arr:any = array.substring(1, array.length - 1).split(",");
    let mastrray:any = [];
    let cont_exec:boolean = true;
    for (let i = 0; i < arr.length; i++) {
