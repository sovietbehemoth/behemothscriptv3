...
async function parsearraystr(array:string, name:string, type:types, push?:order):Promise<any> {
...
async function parsearrayint(array:string, name:string, type:types, push?:order) {
    const arr:any = array.substring(1, array.length - 1).split(",");
    let mastrray:any = [];
    let cont_exec:boolean = true;
    for (let i = 0; i < arr.length; i++) {
...
function array_init(content:string):void {
    ...
    typecheck(arrtype);
    if (arrtype === "string") parsearraystr(arr, arrname, arrtype, "push");
    else if (arrtype === "int") parsearrayint(arr, arrname, arrtype, "push");
}
