import { TYPEDEF_STRUCT_STACK, RESTRICT_TYPE_STACK,OBJECTTYPE_DEF_STACK,STRTYPE_DEF_STACK,INTTYPE_DEF_STACK, stackrm, errorf } from "../lexer.ts";
...
async function typecall_init(content:string, push?:boolean): Promise<void> {
    ...
    for (let i = 0; i < RESTRICT_TYPE_STACK.length; i++) {
        if (RESTRICT_TYPE_STACK[i][1] === type) {
            if (RESTRICT_TYPE_STACK[i][0] === "string") {
                ...
                if (push === undefined) STRTYPE_DEF_STACK.push([val,content.split(type)[1].split("=")[0].trim()]);
                return;
                ...
                if (dec === false) errorf(content, "Value does not conform to set restriction", def.join("").trim());
                if (push === undefined) INTTYPE_DEF_STACK.push([val,content.split(type)[1].split("=")[0].trim()]);
                return;
            }
        }
    }
