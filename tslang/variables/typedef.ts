import { TYPEDEF_STRUCT_STACK, RESTRICT_TYPE_STACK,OBJECTTYPE_DEF_STACK,STRTYPE_DEF_STACK,INTTYPE_DEF_STACK, stackrm } from "../lexer.ts";
...
async function typedef_init(content:string, doc:any): Promise<number> {
    ...
    for (let i = 0; i < exec_split.length; i++) {
        ...
        if (type.startsWith("*")) {
            ...
            if (loc === false) throw "ParserError: Nondefault type restriction not found";
...
async function typecall_init(content:string): Promise<void> {
    ...
    for (let i = 0; i < RESTRICT_TYPE_STACK.length; i++) {
        if (RESTRICT_TYPE_STACK[i].type === type) {
            if (RESTRICT_TYPE_STACK[i].type === "string") {
                ...
                if (dec === false) throw "ParserError: Value does not conform to set restriction";
...
async function typedef_access_init(content:string,typedef:any): Promise<void> {
    ...
    if (loc === false) throw "ParserError: Stack scan fault";
