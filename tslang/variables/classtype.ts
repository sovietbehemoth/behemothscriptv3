...
import { INTTYPE_DEF_STACK, OBJECTTYPE_DEF_STACK, __LINE__, errorf } from "../lexer.ts";
...
async function method_init(content:string,doc:any,visibility:class_method_types,typarr:any):Promise<method_init_return>{
    ...
    const rettype:string = content.split("").slice(content.lastIndexOf(":")).join("");
    ...
    if (await isfunction(equto) === true) {
        for (let i = 0; i < CLASSTYPE_DEF_STACK.length; i++) {
            ...
            if (CLASSTYPE_DEF_STACK[i].name.trim() === equto.split("(")[0].trim()) {
                ...
                if (CLASSTYPE_DEF_STACK[i].constructor.present === false) errorf(contentf, "Class does not have a constructor");
                else {
                ...
                for (let i3 = 0; i3 < CLASSTYPE_DEF_STACK.length; i3++) {
                    ...
                    if (CLASSTYPE_DEF_STACK[i3].name.trim() === OBJECTTYPE_DEF_STACK[i2].class.name.trim()) {
                        ...
                        if (OBJECTTYPE_DEF_STACK[i2].name.trim() === "behemoth") {
                            return await compiler_call(call);
                        }          
                                ...
                                if (methods[i4].methodname.trim() === call.split("(")[0].trim()) {
                                    if (methods[i4].role === "public") {
                                        ...
                                        for (let i5 = 0; i5 < method_len; i5++) {
                                            if (method_exec[i5].split(" ")[0].trim() === "return") {
                                                if (methods[i4].return_init === "int") retval = await intparse(method_exec[i5].split("return")[1].trim());
                                                if (methods[i4].return_init === "string") retval = await strparse(method_exec[i5].split("return")[1].trim(),true);
                                                if (methods[i4].return_init === "void") retval = undefined;
                                                else if (!methods[i4].return_init.startsWith("*")) await errorf(data, `Illegal return type`, methods[i4].return_init.trim());
                                                return retval;
                                            }
