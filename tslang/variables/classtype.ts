...
import { websocket_constructor_init, socket_method_handler } from "../libs/websockets_lib.ts"
...
async function objparse(content:string, data:any, doc:any): Promise<any> {
    let instr:boolean = false;
    ...
    for (let i = 0; i < content.length; i++) {
        ...
        if (content[i] === "\"" && content[i-1] !== "\\" && instr === true) {
            instr = false;
        }
        if (content[i] === "." && instr === false) {
            main_arr.push(cur_subj.join("").trim());
            cur_subj = [];
        } else cur_subj.push(content[i]);
    } main_arr.push(cur_subj.join("").trim());
    
    let cur_memb:any;
    let cur_obj:any = main_arr[0];
    for (let i = 0; i < main_arr.length; i++) {
        //if (main_arr[i].trim() === "") continue;
        //if (cur_memb !== undefined) console.log(cur_memb, main_arr[i])
        for (let i2 = 0; i2 < OBJECTTYPE_DEF_STACK.length; i2++) {
            if (await isfunction(main_arr[i]) === true) {
                if (main_arr[i].split("(")[0].trim() === OBJECTTYPE_DEF_STACK[i2].name) {
                    if (OBJECTTYPE_DEF_STACK[i2].type === "classinit") {
                        cur_memb = OBJECTTYPE_DEF_STACK[i2];
                        i=i+1;
                        continue;
                    }
                }
            } else {
                if (main_arr[i] === OBJECTTYPE_DEF_STACK[i2].name) {
                    if (OBJECTTYPE_DEF_STACK[i2].type === "classinit") {
                        cur_memb = OBJECTTYPE_DEF_STACK[i2];
                        i=i+1;
                        continue;
        ...
        for (let i2 = 0; i2 < CLASSTYPE_DEF_STACK.length; i2++) {
            if (await isfunction(main_arr[i]) === true) {
                if (main_arr[i].split("(")[0].trim() === CLASSTYPE_DEF_STACK[i2].name) {
                    return CLASSTYPE_DEF_STACK[i2];
                }
            }
        }
        for (let i2 = 0; i2 < FUNCTION_DEF_STACK.length; i2++) {
            if (await isfunction(main_arr[i])) {
                if (main_arr[i].split("(")[0].trim() === FUNCTION_DEF_STACK[i2][0].trim() && cur_memb === undefined) {
                    return await function_call_init(FUNCTION_DEF_STACK[i2], main_arr[i])
                }
            }
        }
        if (cur_memb !== undefined) {
            let break_signal:boolean = false;
            for (let i2 = 0; i2 < cur_memb.class.members.object.length; i2++) {
                const indexxed_obj:string = cur_memb.class.members.object[i2][0];
                //console.log(indexxed_obj)
                if (indexxed_obj === main_arr[i]) {
                    //console.log(cur_memb)
                    cur_memb = cur_memb.class.members.object[i2][1];
                    //console.log(cur_memb)
                    break_signal = true;
                    break;
                }
            ...
            for (let i2 = 0; i2 < cur_memb.class.members.method.length; i2++) {
                const indexxed_method:string = cur_memb.members.method[i2].methodname
                if (indexxed_method === main_arr[i] && await isfunction(main_arr[i]) === true) {
                    const method:any = cur_memb.members.method[i2];
                    if (method.role === "public") {
                        ...
                        if (method.params[0].trim() !== "void") {
                            for (let i5 = 0; i5 < method.params.length; i5++) {
                                const param:string = method.params[i5];
                        ...
                        const method_len:number = method.execution.split(";").length;
                        const method_exec:any = method.execution.split(";");
                        const method_ret:any = method.return_init.trim();
                        ...
                        for (let i5 = 0; i5 < method_len; i5++) {
                            if (method_exec[i5].trim().split(" ")[0].trim() === "return") {
                                ...
                                if (method.return_init.trim() === "int") retval = await intparse(method_exec[i5].split("return")[1].trim());
                                else if (method.return_init.trim() === "string") retval = await strparse(method_exec[i5].split("return")[1].trim(),true);
                                else if (method.return_init.trim() === "void") retval = undefined;
                                else /*if (method.return_init.trim().startsWith("%r"))*/ {
                                    for (let i6 = 0; i6 < RESTRICT_TYPE_STACK.length; i6++) {
                                        if (RESTRICT_TYPE_STACK[i6][1].trim() === method.return_init.trim().split("%r")[1].trim()) {
                                ...
                                //} /*else if (method.return_init.trim().startsWith("%cl")) {*/
                            ...
                            if (method_exec[i5].trim().split(" ")[0].trim() === "this") {
                                if (cur_memb.class.members.int.length > 0) {
                                    for (let i6 = 0; i6 < cur_memb.class.members.int.length; i6++) {
                                        if (cur_memb.class.members.int[i6].name.trim() === method_exec[i5].split("this")[1].trim().split(" ")[0].trim()) {
                                            const equ:string = method_exec[i5].split("=")[1].trim();
                                            cur_memb.class.members.int[i6].value.pop();
                                            cur_memb.class.members.int[i6].value.push(await intparse(equ));
                                            haltorder = true;
                                            //console.log(cur_memb.class.members.int[i6].value);
                                        }
                                    }
                                }
                                for (let i6 = 0; i6 < cur_memb.class.members.object.length; i6++) {
                                    if (cur_memb.class.members.object[i6].name.trim() === method_exec[i5].split("this")[1].trim().split(" ")[0].trim()) {
                                        const equ:string = method_exec[i5].split("=")[1].trim();
                                        cur_memb.class.members.object[i6].value.pop();
                                        //console.log(await init_new_obj(equ,data,doc));
                                        cur_memb.class.members.object[i6].value.push(await init_new_obj(equ,data,doc));
                                        haltorder = true;
                                        //console.log(CLASSTYPE_DEF_STACK[i].members.int[i3].value);
                                    }
                                }
                                for (let i6 = 0; i6 < cur_memb.class.members.string.length; i6++) {
                                    if (cur_memb.class.members.string[i6].name.trim() === method_exec[i5].split("this")[1].trim().split(" ")[0].trim()) {
                                        const equ:string = method_exec[i5].split("=")[1].trim();
                                        cur_memb.class.members.string[i6].value.pop();
                                        cur_memb.class.members.string[i6].raw.pop()
                                        cur_memb.class.members.string[i6].raw.push(equ);
                                        cur_memb.class.members.string[i6].value.push(await strparse(equ,true));
                                        haltorder = true;
                                ...
                                if (method_exec[i5].split("=")[1].trim().split(" ")[0].trim() === "this") {
                                    for (let i6 = 0; i6 < cur_memb.class.members.int.length; i6++) {
                                        if (cur_memb.class.members.int[i6].name.trim() === method_exec[i5].split("this")[1].trim().split(" ")[0].trim()) {
                                            await lexer_exec(`${method_exec[i5].split("=")[0].trim()} = ${cur_memb.class.members.int[i6].value}`, doc, "class");
                                            haltorder = true;
                                        }
                                    }
                                    for (let i6 = 0; i6 < cur_memb.class.members.string.length; i6++) {
                                        if (cur_memb.class.members.string[i6].name.trim() === method_exec[i5].split("this")[1].trim().split(" ")[0].trim()) {
                                            //console.log(`${method_exec[i5].split("=")[0].trim()} = "${cur_memb.class.members.string[i6].value}"`);
                                            await lexer_exec(`${method_exec[i5].split("=")[0].trim()} = "${cur_memb.class.members.string[i6].value}"`, doc, "class");
                                            haltorder = true;
                                        }
                                    }
                                }
            ...
            /*for (let i2 = 0; i2 < cur_memb.class.members.object.length; i2++) {
                const indexxed_object:string = cur_memb.class.members.object[i2].name
                console.log(indexxed_object, main_arr[i])
                if (indexxed_object === main_arr[i]) {
                    cur_memb = cur_memb.class.members.object[i2];
                    continue;
                }
            }*/
            for (let i2 = 0; i2 < cur_memb.class.members.int.length; i2++) {
                const indexxed_int:string = cur_memb.class.members.int[i2].name
                if (indexxed_int === main_arr[i]) {
                    return cur_memb.class.members.int[i2].value[0];
                }
            }
            for (let i2 = 0; i2 < cur_memb.class.members.string.length; i2++) {
                const indexxed_str:string = cur_memb.class.members.string[i2].name;
                //console.log(indexxed_str);
                if (indexxed_str === main_arr[i]) {
                    return cur_memb.class.members.string[i2].value[0];
                }
            }
            for (let i2 = 0; i2 < cur_memb.class.members.array.length; i2++) {
                const indexxed_arr:string = cur_memb.class.members.array[i2].name
                if (indexxed_arr === main_arr[i]) {
                    return cur_memb.class.members.array[i2].value;
                }
            }
        } 
    }
                ...
                for (let i3 = 0; i3 < CLASSTYPE_DEF_STACK.length; i3++) {
                    ...
                    if (CLASSTYPE_DEF_STACK[i3].name.trim() === OBJECTTYPE_DEF_STACK[i2].class.name.trim()) {
                        ...
                        if (internals !== false || internals === undefined) {
                            ...
                            } else if (OBJECTTYPE_DEF_STACK[i2].class.name === "WebSocket") {
                                //console.log(OBJECTTYPE_DEF_STACK[i2])
                                await socket_method_handler(OBJECTTYPE_DEF_STACK[i2].class.members.object[0].wsobj, call);
                                return 0;
                            }
                        }
                        if (await isfunction(call) === true) {
                            ...
                            for (let i4 = 0; i4 < CLASSTYPE_DEF_STACK[i3].members.object.length; i4++) {
                                if (CLASSTYPE_DEF_STACK[i3].members.object[i4].name.trim() === call) {
                                    if (data.includes("=")) {
                                        if (data.split("=")[0].trim().includes(init_word)) {
                                            if (data.split(call.trim())[1].includes(".")) {
...
async function class_master_creator(contentf:string,doc:any, push?:boolean):Promise<any> {
    ...
    let classif:any;
    //console.log(equto)
    //console.log(await objparse(equto,equto,doc));
    if (contentf.includes(".")) {
        for (let i = 0; i < OBJECTTYPE_DEF_STACK.length; i++) {
            ...
            if (OBJECTTYPE_DEF_STACK[i].name.trim() === contentf.split(".")[0].trim().split("=")[1].trim() && OBJECTTYPE_DEF_STACK[i].type === "classinit") {
                ...
                if (itemf === "http") {
                    //console.log(equto.slice(equto.indexOf(".")).trim())
                    const res:any = await http_call(equto.slice(equto.indexOf(".")+1).trim(), "");
                    //console.log(res);
                    const struct:class_init_pushed = {
                        type: "classinit",
                        name: objname,
                        class: res.class
                    }
                    OBJECTTYPE_DEF_STACK.push(struct);
                } else if (itemf === "behemoth") {
                    await compiler_call(equto.split(".").slice(1).join("").trim())
                } else if (itemf === "sys") {
                    await os_class_call(equto.split(".").slice(1).join("").trim(), "");
                } else if (OBJECTTYPE_DEF_STACK[i].class.name === "WebSocket") {
                    //console.log(OBJECTTYPE_DEF_STACK[i])
                    await socket_method_handler(OBJECTTYPE_DEF_STACK[i].class.members.object[0].wsobj,equto.split(".").slice(1).join("").trim() );
                    return 0;
                }
                //console.log();
            ...
            if (CLASSTYPE_DEF_STACK[i].name.trim() === equto.split("(")[0].trim()) {
                ...
                if (CLASSTYPE_DEF_STACK[i].constructor.present === false) {}
                   ...
                   //console.log(exdata);
                    ...
                    for (let i2 = 0; i2 < exdata.length; i2++) {
                        if (exdata[i2].split(" ")[0].trim() === "this") {
                            ...
                            for (let i3 = 0; i3 < CLASSTYPE_DEF_STACK[i].members.object.length; i3++) {
                                //console.log("FWLLL");
                                if (CLASSTYPE_DEF_STACK[i].members.object[i3].name === exdata[i2].split("this")[1].trim().split(" ")[0].trim().split(".")[0]) {
                                    const equ:string = exdata[i2].split("=")[1].trim();
                    ...
                    if (CLASSTYPE_DEF_STACK[i].name === "WebSocket") {
                        await websocket_constructor_init(name.join(""), fullcon);
                    }
        ...
        if (found === false) {
            const fname:string = content.slice(content.indexOf("=")+1).split("(")[0].trim()
            for (let i0 = 0; i0 < FUNCTION_DEF_STACK.length; i0++) {
                if (FUNCTION_DEF_STACK[i0][0].trim() === fname) {
                    const res:any = await function_call_init(FUNCTION_DEF_STACK[i0],content.slice(content.indexOf("=")+1));
                    const psh:class_init_pushed = {
                        name: objname,
                        type: "classinit",
                        class: res
                    }; OBJECTTYPE_DEF_STACK.push(psh);
                }
            }
        }
    }
...
async function objtype_member_init(contentf:string, doc:string, visibility:class_method_types): Promise<any> {
    ...
    return [await objparse(pass.join("").trim(),pass.join("").trim(),doc),name.join("").split(`${visibility} object`)[1].trim()];
    console.log(await objparse(pass.join("").trim(),pass.join("").trim(),doc))
...
export { class_init, class_master_creator, init_new_obj, objparse, CLASS_TEMPORARY_DATA };
export type { master_class, method_init_return, class_init_pushed, strtype_member, inttype_member, arrtype_member };