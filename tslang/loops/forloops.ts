import { lexer_exec, stackrm, INTTYPE_DEF_STACK, ARRTYPE_DEF_STACK, STRTYPE_DEF_STACK, FUNCTION_DEF_STACK, errorf } from "../lexer.ts";
 ...
 */
async function for_init(content:string, doc:string): Promise<number> {
    let poporders:any = [];
    ...
    while (await calculate_condition(fullcon[1].trim()) === true) {
        //console.log(await calculate_condition(fullcon[1].trim()), await intparse(fullcon[1].trim().split("<")[0].trim()));
        ...
        for (let i = 0; i < execution_data.split(";").length-1; i++) {
            //console.log(execution_data.split(";")[i].length);
        ...
        //Poporders array and pop loop scopes variables
