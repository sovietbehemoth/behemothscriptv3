import {__DEBUG__,errorf} from "./lexer.ts";
...
let LEFT_BRACE_STACK:Array<any> = [];
    ...
    //**Scan all braces and push them to stack (DOES NOT IDENTIFY SYNTAX ERRORS) */
    ...
    function check_brace_syntax(): void {
        if (LEFT_BRACE_STACK.length !== RIGHT_BRACE_STACK.length) {
            let LGAR:any;
            if (LEFT_BRACE_STACK.length > RIGHT_BRACE_STACK.length) LGAR = LEFT_BRACE_STACK;
            else LGAR = RIGHT_BRACE_STACK.length;

            const badbrace = LGAR[LGAR.length-1].split(":")[0];
            const badbracepos = LGAR[LGAR.length-1].split(":")[1];

            errorf(badbrace,`Unexpected ${badbrace} on line ${badbracepos}`,badbrace);
        }
    }


export { bracer_master, locate_opening_bracket, check_brace_syntax };