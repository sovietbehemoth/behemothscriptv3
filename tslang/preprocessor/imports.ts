...
async function read_file_prep(content: string,doc:any) {
    ...
    if (content.includes('"')) {
        ...
        const __intern__:any = new Boolean(importf === "standard" || importf === "socket" || importf === "requests" || importf === "system");
        if (__intern__ != true) errorf(content, "Unrecognized internal import", importf, "PreprocessorError"); else {
            switch (importf) {
                ...
                case "system":
                    ...
                    break;
                case "socket":
                    const rimpls:any = await Deno.readTextFile("./libs/socket.bhs");
                    await lexer_init(rimpls.replace(/[\r\n]+/gm, ""),"import");
                    break;
            }
        }
    }
