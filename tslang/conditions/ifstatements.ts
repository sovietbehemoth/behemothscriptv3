...
import { lexer_exec, errorf, BRACE_SYSTEM } from "../lexer.ts";
...
async function else_if_conditional_statement_init(content:string, doc:any): Promise<void> {
    if (PRECONDITION === "") await errorf(content,"No conditional statement preceding 'elif'","elif");
    const execution_data = doc.substring(BRACE_SYSTEM.locate_opening_bracket(content, doc)[0] + 1, BRACE_SYSTEM.locate_opening_bracket(content, doc)[1]).trim();
    const condition = content.split("if")[1].split("{")[0].trim().substring(1, content.split("if")[1].split("{")[0].trim().length - 1);
...
async function else_conditional_statement_init(content:string, doc:any): Promise<void> {
    if (PRECONDITION === "") await errorf(content, "No conditional statement preceding 'else'", "else");
    const execution_data = doc.substring(BRACE_SYSTEM.locate_opening_bracket(content, doc)[0] + 1, BRACE_SYSTEM.locate_opening_bracket(content, doc)[1]).trim();
